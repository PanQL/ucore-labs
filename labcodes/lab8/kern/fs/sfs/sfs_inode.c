...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    struct sfs_disk_inode *din = sin->din;  //获得磁盘的inode
    assert(din->type != SFS_TYPE_DIR);	// 确保获得的inode是文件，不是目录
    off_t endpos = offset + *alenp, blkoff; //endpos获得当前需要读取/写入的内容的最后一个位置
    *alenp = 0;		// 将传入的长度参数置为0,可以起到类似返回值以及同步调用者与被调用者的作用?
	// calculate the Rd/Wr end position
    if (offset < 0 || offset >= SFS_MAX_FILE_SIZE || offset > endpos) { //出现了不合法的调用，读取/写入的长度或位置不合法
        return -E_INVAL;
    }
    if (offset == endpos) { //需要读取/写入的内容为0,直接返回0即可
        return 0;
    }
    if (endpos > SFS_MAX_FILE_SIZE) {
        endpos = SFS_MAX_FILE_SIZE;
    }
    if (!write) {	// to read, not write
        if (offset >= din->size) {
            return 0;
        }
        if (endpos > din->size) {
  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	...
	*/
    ...
    // 先读/写第一部分未对齐的内容，从offset---第一个块结束/endpos.
    if(nblks == 0){
        alen = endpos - offset;
        if((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0){
			goto out;
		}
        if((ret = sfs_buf_op(sfs, buf, alen, ino, blkoff)) != 0){
			goto out;
		}
        goto out;
    }else if(blkoff != 0){
        alen = SFS_BLKSIZE - blkoff;
        if((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0){
			goto out;
		}
        if((ret = sfs_buf_op(sfs, buf, alen, ino, blkoff)) != 0){
    ...
    // 读取最后一部分没有对齐的内容，从某一磁盘块起始位置到endpos
