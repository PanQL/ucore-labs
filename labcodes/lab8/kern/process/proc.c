...
load_icode(int fd, int argc, char **kargv) {
    ...
    struct mm_struct *mm;
    assert((mm = mm_create()) != NULL);
    int ret = -1;
    if((ret = setup_pgdir(mm)) != 0){
        return ret;
    }

    struct elfhdr _ehdr , *ehdr = &_ehdr;
    struct proghdr _ph, *ph = &_ph;
    struct Page *page;

    if((ret = load_icode_read(fd, ehdr, sizeof(struct elfhdr), 0)) != 0){
        return ret;
    }
    ...
    uint32_t vmflags = 0, perm = 0;
    for(idx = 0; idx < ehdr->e_phnum; idx++){
        ph_off = ehdr->e_phoff + idx * sizeof(struct proghdr);
        ...
        if((ret = mm_map(mm, ph->p_va, ph->p_memsz, vmflags, NULL)) != 0){
            return ret;
        }
        ...
        uint32_t size;
        uint32_t page_offset, p_offset = ph->p_offset;
        while(start < end){
            page_offset = start - la;
            size = PGSIZE - page_offset;
            if(ROUNDDOWN(end, PGSIZE) == la){
                size -= PGSIZE - end + la;
            }
            page = pgdir_alloc_page(mm->pgdir, la, perm);
            if((ret = load_icode_read(fd, page2kva(page) + page_offset, size, p_offset)) != 0){
                return ret;
            }
        ...
        if(start < la){
            ...
            memset(page2kva(page) + page_offset, 0, size);
            start += size;
            la += PGSIZE;
        }
    ...
    // 赋予该用户程序使用堆栈的权限
    vmflags = VM_READ | VM_WRITE | VM_STACK;
    if ((ret = mm_map(mm, USTACKTOP - USTACKSIZE, USTACKSIZE, vmflags, NULL)) != 0) {
        return ret;
    }
    mm_count_inc(mm);
    current->mm = mm;
    ...
    argv_size = 0;
    for (i = 0; i < argc; i ++) {   // 拷贝参数字符串，并将对应的字符指针入栈
        uargv[i] = strcpy((char *)(stacktop + argv_size ), kargv[i]);
        argv_size +=  strnlen(kargv[i],EXEC_MAX_ARG_LEN + 1)+1;
    }
    
    stacktop = (uintptr_t)uargv - sizeof(int);
    *(int *)stacktop = argc;    // 传入参数个数
    
    // 设置中断帧，用于切换特权级，并设置新的pc，响应中断结束之后可进入用户态执行加载的程序
    struct trapframe *tf = current->tf;
    memset(tf, 0, sizeof(struct trapframe));
    tf->tf_cs = USER_CS;
    tf->tf_ds = tf->tf_es = tf->tf_ss = USER_DS;
    tf->tf_esp = stacktop;
    tf->tf_eip = ehdr->e_entry;
    tf->tf_eflags = FL_IF;
    ret = 0;
    return ret;
